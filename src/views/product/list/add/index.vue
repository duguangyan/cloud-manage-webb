    ...
    >
      <el-card class="box-card">
        ...
        <div class="text item">
            ...
            >
              ...
              <template v-else-if="item.inputType === 1">
                <el-radio-group v-model="addForm.generate[index].list" size="small">
                  <el-radio
                    v-for="(radioItem, radioIndex) in item.valueSet"
                    :key="radioIndex"
              ...
              <template v-else-if="item.inputType === 2">
                ...
                >
                  <el-checkbox
                    v-for="(checkboxItem, checkboxIndex) in item.valueSet"
                    :label="checkboxItem.value"
                    :key="checkboxIndex"
                  >{{checkboxItem.value}}</el-checkbox>
                </el-checkbox-group>
                <el-checkbox
                  :indeterminate="isIndeterminate"
                  v-model="addForm.generate[index].checkAll"
                  @change="((val) => handleCheckAllChange(val, index, item.id))"
                >全选</el-checkbox>
                <span v-if="item.exp !== null">{{item.exp}}</span>
              </template>
              <template v-else-if="item.inputType === 3">
                ...
                >
                  <el-option
                    v-for="(selectItem, selectIndex) in item.valueSet"
                    :key="selectIndex"
                    :label="selectItem.id"
              ...
              <template v-else-if="item.inputType === 4">
                <el-input
                  class="long-input"
                  v-model.trim="addForm.generate[index].list"
                  size="medium"
                  maxlength="64"
                  :placeholder="item.hint"
                  :style="{width: item.length + 'px'}"
          ...
          <el-form-item
            v-for="(selfItem, selfIndex) in addForm.selfProp"
            :key="selfIndex + 'x'"
            :label="addForm.selfProp[selfIndex].name"
          >
            ...
            <el-button
              ...
              size="small"
              v-waves
              @click="removeSelfProp(selfIndex)"
            >删除</el-button>
          </el-form-item>
          <el-form-item label required prop="title">
            <el-button size="medium" type="primary" plain @click="addSelfProp">新增属性</el-button>
          </el-form-item>
        </div>
      </el-card>
      <el-card class="box-card">
        ...
        <el-tabs v-loading="moreLoading" v-model="activeName" @tab-click="handleClick">
          <el-tab-pane label="默认报价方式" name="first">
            <div class="text item">
              <el-form-item
                label="计量单位"
                :rules="{required: true, message: `${item.name}必填`, trigger: 'blur', type: item.inputType === 0 || item.inputType === 2 ? 'array' : ''}"
                :prop="'generate.' + index + '.list'"
              >
                <template v-if="item.inputType === 0">
                  <el-cascader
        ...
        <el-tabs v-model="activeName" @tab-click="handleClick">
          <el-tab-pane label="默认报价方式" name="first">
            <div class="text item">
              ...
              >
                <el-select
                  ...
                  maxlength="64"
                  placeholder="请选择"
                  @change="((val) => unitChange(val, 'auto'))"
                >
                  <el-option
              ...
              <el-form-item>
                ...
                <div v-if="tableShow" class="el-form-item__error">请填写完整表格信息</div>
              </el-form-item>

              <el-form-item>
                <el-button
                  v-show="moreSpecTableShow"
                  type="primary"
                  plain
                  size="medium"
                  v-waves
              ...
              >
                <el-option
                  v-for="(item, index) in sellSpeData"
                  :key="index"
                  :label="item.name"
                  :value-key="item.id"
                  :value="item.id"
            ...
            <el-form-item>
              ...
              <div v-if="moreTableShow" class="el-form-item__error">请填写完整表格信息</div>
            </el-form-item>
          </el-tab-pane>
        </el-tabs>
      </el-card>
    ...
    <div class="bottom-box">
      <div>
        ...
        <el-button
          ...
          v-waves
          class="filter-item"
          @click="submitForm('productForm', 1)"
        >上架出售</el-button>
      </div>
    </div>
...
let vm = {
  ...
  methods: {
    ...
    handleClick(tab, event) {
      ...
      if (this.activeName === "second") {
        ...
        getUnitList({ categoryId: this.categoryId })
          ...
          .catch(err => [(this.moreLoading = false)]);
      }
    },
    getAddress() {
      // 获取产地信息
    ...
    addStair(index, id) {
      ...
      if (currentNum && currentPrice) {
        if (index > 0) {
          ...
          if (
            Number(currentNum) <= Number(preNum) ||
            Number(currentPrice) >= Number(prePrice)
          ) {
            this.$message({
              type: "warning",
              message: `阶梯${index +
                1}的起批数必须比阶梯${index}的起批数大，阶梯${index +
                1}的单价必须比阶梯${index}的单价小!`
            });
            return;
          }
        }
    ...
    checkChange(value, index) {
      ...
      this.addForm.generate[index].checkAll =
        checkedCount === this.checkboxObj[index].length;
    ...
    async addMoreSpec() {
      ...
      if (this.sellSpeData.length === 0) {
        this.moreLoading = true;
        await getSpeList({ categoryId: this.categoryId })
          .then(res => {
            this.moreLoading = false;
            if (Array.isArray(res.data)) {
